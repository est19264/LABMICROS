...
reinicio_tmr0 MACRO ; Implementación de MACRO para reiniciar el tmr0
    ...
    bcf ((INTCON) and 07Fh), 2 ; Vuelve 0 al bit de overflow
...
decimal:
    clrf centena ; Empieza la parte de las centenas
    movf PORTA, 0 ; Se mueve lo que hay en el contador a w
    movwf residuo ; Se mueve w a la variable residuos
    movlw 100 ; Se mueve 100 a w
    subwf residuo, 0 ; A residuos se le resta 100
    btfsc STATUS, 0 ; Se verifica si la bandera de status es 0
    incf centena ; Se incrementa una variable si la bandera es 1
    btfsc STATUS, 0 ; Se vuelve a verificar la bandera
    movwf residuo ; Se mueve lo sobrante a residuos
    btfsc STATUS, 0 ;
    goto $-7 ;

    clrf decena ; Empieza la parte de las decenas
    ...
    btfsc STATUS, 0 ; Se verifica la bandera
    movwf residuo ; Se usa residuos como unidades ya que es lo que sobra
    btfsc STATUS, 0 ; Se verifica la bandera
    goto $-7 ;
    btfss STATUS, 0 ;
    return
...
timer0:
    ...
    bsf ((OPTION_REG) and 07Fh), 0 ; PS = 111
    banksel PORTA
    reinicio_tmr0
    return
