...
isr:
    ...
    BANKSEL PORTB
    btfsc ((PIR1) and 07Fh), 0 ; Se revisa si hay overflow en el timer1
    ...
    BANKSEL PIR1
    btfsc ((PIR1) and 07Fh), 1 ; Se revisa si hay overflow en el timer2
...
; Se manda a llamar la configuración para el clock
    call clock

; Configuración para timer0
    BANKSEL OPTION_REG
    bcf ((OPTION_REG) and 07Fh), 5
    bcf ((OPTION_REG) and 07Fh), 3 ; Se utilza el prescaler del timer0
    bsf ((OPTION_REG) and 07Fh), 0 ; Prescaler 11 = 1:256
    bsf ((OPTION_REG) and 07Fh), 1
    bsf ((OPTION_REG) and 07Fh), 2

; Configuración para timer1
    BANKSEL T1CON
    bsf ((T1CON) and 07Fh), 5 ; Prescaler 11 = 1:8
    bsf ((T1CON) and 07Fh), 4
    bcf ((T1CON) and 07Fh), 1 ; Se utiliza el internal clock
    bsf ((T1CON) and 07Fh), 0 ; Se habilita el timer1
...
; Interrupciones
    ...
    bsf ((INTCON) and 07Fh), 5 ; Interrupcion para timer0
    bcf ((INTCON) and 07Fh), 2

    BANKSEL PIE1
    bsf ((PIE1) and 07Fh), 1 ; Se habilita timer2 para pr2
    bsf ((PIE1) and 07Fh), 0 ; Se habilita la interrupcion por overflow para timer1

    BANKSEL PIR1
    bcf ((PIR1) and 07Fh), 1 ; Se limpia la bandera para timer2
    bcf ((PIR1) and 07Fh), 0 ; Se limpia la bandera de interrupcion para timer1
